...
const HealthTrendsGraph = ({ goalFactor }) => {
  ...
  const chartData = useMemo(() => {
    return summary.map((item) => {
      const avgValue = Number(
        item.averageValue ?? item.avgValue ?? item.average ?? item.value ?? 0
      );

      const readingScore = clamp(100 - Math.abs(avgValue - 72) * 2, 0, 100);

      const compositeScore = readingScore * 0.7 + goalFactor * 100 * 0.3;

      return {
        date: item.date ?? item.recordedDate ?? "",

        averageValue: avgValue,

        readingScore,

        score: Math.round(clamp(compositeScore, 0, 100)),
      };
    });
  }, [summary, goalFactor]);
    ...
    return (
      <section className="dashboard-card trends-card">
        <div className="card-header">
          <div>
            <p className="card-eyebrow">HEALTH TRENDS</p>

            <h2>Daily Health Consistency</h2>
          </div>

          <span className="status-badge status-green">Target 72 BPM</span>
        </div>
    ...
    return (
      <section className="dashboard-card trends-card">
        ...
        <div className="chart-empty">
          ...
          <p>No heart rate data available for analysis</p>
        </div>
      </section>
    );
  }
  ...
  const getY = (score) => {
    return paddingTop + ((100 - score) / 100) * chartHeight;
  };
      ...
      <div className="card-header">
        <div>
          ...
          <p className="card-description">
            A combined view of reading stability and goal progress.
          </p>
        </div>
...
const IndividualDashboard = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const fetchGoalSummary = async () => {
      try {
        ...
        console.error("Failed to fetch goal summary:", error);
  ...
  const total = goalSummary?.totalGoals ?? 0;

  const achieved = goalSummary?.achievedGoals ?? 0;

  const averageProgress = clamp(
    Number(goalSummary?.averageProgress ?? 0),
    0,
    1
  );

  const percentage = Math.round(averageProgress * 100);
  ...
  return (
    <main className="dashboard-page">
      ...
      <section className="dashboard-hero">
        <div>
          ...
          <p className="hero-subtitle">
            Your health metrics and goals are being monitored.
          </p>
        </div>
      </section>
      ...
      <section className="dashboard-grid">
        ...
        <HealthTrendsGraph goalFactor={averageProgress} />
        ...
        <section className="dashboard-card goal-card">
          <div className="card-header">
            <div>
              <p className="card-eyebrow">GOALS</p>
          ...
          {goalLoading ? (
            ...
            <>
              <div className="goal-percentage">
                ...
                <small>overall achievement</small>
              </div>
              ...
              <div className="goal-footer">
                ...
                <span className="goal-status">
                  {total === 0 ? "No goals yet" : `${percentage}%`}
                </span>
              </div>
            </>
          )}
